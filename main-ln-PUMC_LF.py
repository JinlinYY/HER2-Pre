...
from metrics.print_metrics import print_average_metrics, print_mean_std_metrics, print_average_metrics_2
...
from metrics.plot_dca_curve import plot_dca_curve, plot_dca_curve_2
...
def main():
    ...
    # 检查是否已经保存了融合后的特征
    if os.path.exists('PUMC_LF_ln_combined_features_tensor.pth') and os.path.exists('PUMC_LF_ln_label_tensor.pth'):
        print("Loading pre-saved features and labels...")
        combined_features_tensor = torch.load('PUMC_LF_ln_combined_features_tensor.pth')
        label_tensor = torch.load('PUMC_LF_ln_label_tensor.pth')
    else:
        # Load data
        index, excel_feature, label = extract_excel_features('/tmp/pycharm_project_600/PUMC_LF/test-ln.xlsx')
        ...
        # 保存融合后的特征和标签
        torch.save(combined_features_tensor, 'PUMC_LF_ln_combined_features_tensor.pth')
        torch.save(label_tensor, 'PUMC_LF_ln_label_tensor.pth')
    ...
    for train_index, test_index in skf.split(combined_features, label):
        ...
        net = TransformerClassifier(input_dim=combined_features.shape[1], num_heads=1, num_encoder_layers=1, dim_feedforward=256, output_size=2)
        ...
        # dca_fold = plot_dca_curve_2(y_test, test_probs, dataset_type=f"Fold {fold} Test")
        plot_dca_curve_2(y_test, test_probs, dataset_type=f"Fold {fold} Test", save_path=f"DCA_fig/dca_curve_fold{fold}.png")
        roc_auc_fold = plot_roc_curve_2(y_test, test_probs, dataset_type=f"Fold {fold} Test", save_path=f"ROC_fig/roc_curve_fold{fold}.png")
        # roc_auc_fold = plot_roc_curve_2(y_test, test_probs, dataset_type=f"Fold {fold} Test")
    ...
    overall_roc_auc = plot_roc_curve_2(all_y_true, all_y_probs, dataset_type="Overall", save_path="ROC_fig/roc_curve.png")
    # overall_roc_auc = plot_roc_curve(all_y_true, all_y_probs, dataset_type="Overall")
    # print(overall_roc_auc)

    # Draw overall DAC curve
    overall_dac_metrics = plot_dca_curve_2(all_y_true, all_y_probs, dataset_type="Overall", save_path="DCA_fig/dac_curve_net_benefit.png")
    ...
    print_average_metrics_2(accuracy_scores, precision_scores, recall_scores, f1_scores, roc_auc_scores, specificity_scores, FNR_scores)
