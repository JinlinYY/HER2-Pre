...
def main():
    ...
    # 检查是否已经保存了融合后的特征
    if os.path.exists('PUMC_ln_combined_features_tensor.pth') and os.path.exists('PUMC_ln_label_tensor.pth'):
        print("Loading pre-saved features and labels...")
        combined_features_tensor = torch.load('PUMC_ln_combined_features_tensor.pth')
        label_tensor = torch.load('PUMC_ln_label_tensor.pth')
    else:
        # Load data
        ...
        # 保存融合后的特征和标签
        torch.save(combined_features_tensor, 'PUMC_ln_combined_features_tensor.pth')
        torch.save(label_tensor, 'PUMC_ln_label_tensor.pth')
