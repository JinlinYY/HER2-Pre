...
def main():
    ...
    # 检查是否已经保存了融合后的特征
    if os.path.exists('HER2_PUMC_combined_features_tensor.pth') and os.path.exists('HER2_PUMC_label_tensor.pth'):
        print("Loading pre-saved features and labels...")
        combined_features_tensor = torch.load('HER2_PUMC_combined_features_tensor.pth')
        label_tensor = torch.load('HER2_PUMC_label_tensor.pth')
    else:
        # Load data
        ...
        # 保存融合后的特征和标签
        torch.save(combined_features_tensor, 'HER2_PUMC_combined_features_tensor.pth')
        torch.save(label_tensor, 'HER2_PUMC_label_tensor.pth')
