...
from metrics.plot_roc_curve import plot_roc_curve_2
...
from metrics.plot_dca_curve import plot_dca_curve_2
from metrics.compute_specificity_fnr import compute_specificity_fnr_2
...
def main():
    ...
    # 检查是否已经保存了融合后的特征
    if os.path.exists('ln_pumc_lf_combined_train_features_tensor.pth') and os.path.exists('ln_pumc_lf_train_label_tensor.pth') and \
            os.path.exists('ln_pumc_lf_combined_val_features_tensor.pth') and os.path.exists('ln_pumc_lf_val_label_tensor.pth') and \
            os.path.exists('ln_pumc_lf_combined_test_features_tensor.pth') and os.path.exists('ln_pumc_lf_test_label_tensor.pth'):
        print("Loading pre-saved features and labels...")
        combined_train_features_tensor = torch.load('ln_pumc_lf_combined_train_features_tensor.pth')
        train_label_tensor = torch.load('ln_pumc_lf_train_label_tensor.pth')
        combined_val_features_tensor = torch.load('ln_pumc_lf_combined_val_features_tensor.pth')
        val_label_tensor = torch.load('ln_pumc_lf_val_label_tensor.pth')
        combined_test_features_tensor = torch.load('ln_pumc_lf_combined_test_features_tensor.pth')
        test_label_tensor = torch.load('ln_pumc_lf_test_label_tensor.pth')
    else:
        # Load training data (Excel)
        train_index, train_excel_feature, train_label = extract_excel_features(
            '/tmp/pycharm_project_600/PUMC_LF/副本编码后汇总-协和+隆福-淋巴结分类-split_data71515-train-del.xlsx')
        ...
        val_index, val_excel_feature, val_label = extract_excel_features(
            '/tmp/pycharm_project_600/PUMC_LF/副本编码后汇总-协和+隆福-淋巴结分类-split_data71515-val-del.xlsx')
        ...
        test_index, test_excel_feature, test_label = extract_excel_features(
            '/tmp/pycharm_project_600/PUMC_LF/副本编码后汇总-协和+隆福-淋巴结分类-split_data71515-test-del.xlsx')
        ...
        # 保存融合后的特征和标签
        torch.save(combined_train_features_tensor, 'ln_pumc_lf_combined_train_features_tensor.pth')
        torch.save(train_label_tensor, 'ln_pumc_lf_train_label_tensor.pth')
        torch.save(combined_val_features_tensor, 'ln_pumc_lf_combined_val_features_tensor.pth')
        torch.save(val_label_tensor, 'ln_pumc_lf_val_label_tensor.pth')
        torch.save(combined_test_features_tensor, 'ln_pumc_lf_combined_test_features_tensor.pth')
        torch.save(test_label_tensor, 'ln_pumc_lf_test_label_tensor.pth')
    ...
    net = TransformerClassifier(input_dim=combined_train_features_tensor.shape[1], num_heads=5, num_encoder_layers=1,
                                dim_feedforward=256, output_size=2)
    ...
    specificity_val, fnr_val = compute_specificity_fnr_2(y_val, val_probs)  # For validation
    specificity_test, fnr_test = compute_specificity_fnr_2(y_test, test_probs)  # For test

    # ROC curve and AUC for validation and test
    plot_roc_curve_2(y_val, val_probs, dataset_type="Validation", save_path="ROC_fig/roc_curve_val.png")
    plot_roc_curve_2(y_test, test_probs, dataset_type="Test", save_path="ROC_fig/roc_curve_test.png")

    # DCA curve for validation and test
    plot_dca_curve_2(y_val, val_probs, dataset_type="Validation", save_path="DCA_fig/dca_curve_val.png")  # For validation
    plot_dca_curve_2(y_test, test_probs, dataset_type="Test", save_path="DCA_fig/dca_curve_test.png")  # For test
    ...
    # # 之前的代码保持不变
    # test_out = net(x_test)
    # test_probs = F.softmax(test_out, dim=1).cpu().detach().numpy()  # 获取每个样本的类别概率
    # y_test_pred = torch.max(test_out, 1)[1].cpu().numpy()  # 获取每个样本的预测标签
    # Extract image features (all images in one folder)
    test_index, test_excel_feature, test_label = extract_excel_features(
        '/tmp/pycharm_project_600/PUMC_LF/副本编码后汇总-协和+隆福-淋巴结分类-split_data71515-test-del.xlsx')
